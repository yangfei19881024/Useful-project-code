	...
	return function($){	
		$.extend({
			...
			dialog_unblock: function()
			{
				...
				{
					oSection.animate({ marginTop: -($.dialog_config.top - 150), opacity: 0});
				}
				else
				{
					oSection.animate({ top: ($.dialog_config.top + 150), opacity: 0});
				}
			...
			dialog_install: function(opts)
			{
				...
				{
					var content = '<div class="dialog-lightbox-list" data-class="dialog-lightbox-list">'
										+'	<img src="" class="dialog-lightbox-list-image" data-class="dialog-lightbox-list-image" />'
										+'</div>'
										+'	'
										+'<span class="dialog-lightbox-prev" data-class="dialog-lightbox-prev">&lt;</span>'
										+'<span class="dialog-lightbox-next" data-class="dialog-lightbox-next">&gt;</span>';
				}
				else
				{
				...
				var plugs_lightbox_section = '<div class="dialog-lightbox-section" data-class="dialog-lightbox-section">'
											+'	<div class="dialog-lightbox-header" data-class="dialog-lightbox-header">'
											+'		<h2>'+ $.dialog_defaults.title +'</h2>'
											+		dialog_light_close
											+'	</div>'
											+'	'
											+'	<div class="dialog-lightbox-body" data-class="dialog-lightbox-body">'
											+		content
											+'	</div>'
											+'</div>';
			...
			dialog_loadImage: function(loadImage_src, isMove, callback)
			{
				...
				{
					$.dialog_config.imageData = { width: image.width, height: image.height, src: loadImage_src};
			...
			dialog_settings: function(section, imageTag, isMove)
			{
				...
				{
					imageTag.hide().attr('src', $.dialog_config.imageData.src).css({ width: $.dialog_config.width, height: $.dialog_config.height}).fadeIn();		
				}			
				...
				if($.dialog_defaults.isFixed)
				{
					...
					if(isMove)
					{
						section.css({ marginLeft: -$.dialog_config.left, marginTop: -$.dialog_config.top});
					}
					else
					{
						section.animate({ marginLeft: -$.dialog_config.left, marginTop: -$.dialog_config.top}, { queue: false});
					}
				}
				else
				{
					...
					section.css({ position: 'absolute',  marginLeft: 0, marginTop: 0});
					
					if(isMove)
					{
						section.css({ left: $.dialog_config.left, top: $.dialog_config.top});
					}
					else
					{
						section.animate({ left: $.dialog_config.left, top: $.dialog_config.top}, { queue: false});
					}
				}
			...
			dialog_move: function(section, isMove)
			{
				...
				{
					section.css({ marginTop: -($.dialog_config.top - 150)})
						   .animate({ marginTop: -$.dialog_config.top, opacity: 1});
				}
				else if(isMove)
				{
					section.css({ top: ($.dialog_config.top + 150)})
						   .animate({ top: $.dialog_config.top, opacity: 1});
				}
				
				section.animate({ width: $.dialog_config.width, height: $.dialog_config.height}, { queue: false});
			},
