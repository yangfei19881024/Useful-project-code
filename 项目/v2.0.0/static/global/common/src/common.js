define(function(require, exports, module){
	...
	$('a[data-action=footer-complaint]').live('click', function(){
		...
		var isLogin = CMF.Actions.isLogin();
	...
	$('[data-state=pm]').live('click', function(){
		...
		// 监听留言输入框状态
		CMF.Message.enterStatus('#privMsg', 'button-light-disabled');
		...
		$('.message-button > [data-action=submit]').die().live('click', function(){				
			...
			CMF.Message.sendMessage('#MsgReplyInfo', datas, $.unblock);
	...
	$('[data-action=feedback]').live('click', function(){
		...
		// 监听意见反馈的输入状态
		CMF.Message.enterStatus('.suggestion-box', 'button-light-disabled', '意见');
		
		$('#selectSuggestion').selects({
			selects: current_value
		});
		
		var UPLOAD_IMAGE_URI = CMF.Methods.uploadImage('[data-class=suggestUpLoadImageError]');
